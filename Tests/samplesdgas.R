...
options(digits = 10)
data <- read.table("Average Value Weighted Returns Monthly.txt", head = T)
starts <- c()
for(i in 1:ncol(data)){
  j = 1
  while(data[j,i] == -99.99){
    j = j + 1
  }
  start <- c(start, j)
}
max(start)
data <- data[data[,1] > 200001,]
data <- data[,1:8]
data <- demean(data)

probs <- list()
pdf("cluster formation.pdf")
for(pp in 0:71){
  y <- t(data[((2*pp) + 1):((2*pp) + 2),])
  y <- y[2:nrow(y),]
  s = 12
  S = 0.1
  tau = 0.5
  m <- colMins(y)
  M <- diag(rep(1, ncol(y)))
  N <- 10000    # Number of Iterations in gibbs
  y <- as.matrix(y)
  M_y <- ncol(y)
  N_y <- nrow(y)
  ...
  alpha = 10e-2
  colnames(clusters) <- rep(paste("y", as.character(seq(1, nrow(y), by = 1)), sep = ""))
  ...
  for(k in 2:N){
    ...
    for(i in 1:N_y){
       ...
       qj[i] <- q_0(s, y[i,], iterations_m[i, ,k], M)
       qj <- qj/sum(qj)
       ...
       #print(cbind(qj, clusters[k,], iterations_m[i, ,k]))
    }
    print(paste("pp: ", pp, "k = ", k))
  }
  ...
  max <- max(dat)
  min = 0
  plot(dat[,2], ylim = c(min, max), type = "l", main = paste("starting time = ", data[pp*2 + 1,1]), xlab = "Cluster names")
  for(i in 3:ncol(dat)){
    rgb_ <- runif(n = 3, min= 0, max = 255)
  ...
  prob <- matrix(0, ncol = nrow(y), nrow = nrow(y))
  start = 50
  end = N
  for(i in start:end){
    prob <- prob + gen_boolean(clusters[i,])/(end - start + 1)
  }
  probs <- c(probs, list(prob))
  #setwd("C:/Users/Tom Kennes/Desktop/Research Master/Thesis/Functions and Codes")
