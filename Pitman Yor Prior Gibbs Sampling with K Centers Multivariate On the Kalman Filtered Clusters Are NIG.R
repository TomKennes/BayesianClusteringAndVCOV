...
# Set up time windows, order and plot
t = 5
...
for(k in 2:N){
  # Note that I only plugged in the beta here to create a pitman yor process!
  alpha = alpha_ + beta*length(levels(as.factor(as.character(clusters[k-1,]))))
  ...
  for(i in 1:N_y){
    ...
    for(j in 1:length(cluster_names)){
      ...
      if(j != i && markov[j] > beta){
        markov[j] = markov[j] - beta
      }
    }
...
plot.lot(t(y), col = colours, max_colours_value = 30)
