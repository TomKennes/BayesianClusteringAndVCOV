...
rnormal_inverse_gamma <- function(s, S, y, m, tau){
  ...
  for(i in 1:nrow(y)){
    ...
    tmp <- 1/(rgamma(n = ncol(y), shape = (1 + s)/2, rate = diag(S)))
...
y = y[26:(26+t),2:ncol(y)]
plot.lot(y, main = "All Portofolios for Months 197108-197201")
y = t(y)
...
m <- colMeans(y)
#M = (1 + tau)*S/s
N <- 1000  # Number of Iterations in gibbs
...
plot.lot(t(y), col = colours, max_colours_value = 30, main = paste("Last Partition: Clustering of Portfolios, colours are clusters. Cluster NIG Prior", "\n", "Number of clusters: ", as.character(length(types)), " Without Fixed Variance"))
...
C = greedy.assign(MAP.P, P = 0.75)
colours <- matrix(0, nrow = length(C), ncol = 3)
types = as.numeric(levels(as.factor(as.character(C))))
for(i in 1:length(types)){
  col = runif(3, 0, 30)
  for(j in 1:length(C)){
    if(C[j] == types[i]){
...
plot.lot(t(y), col = colours, max_colours_value = 30, main = paste("Greedy MAP: Clustering of Portfolios, colours are clusters. Cluster NIG Prior", "\n", "Number of clusters: ", as.character(length(types)), " Without fixed Variance"))
