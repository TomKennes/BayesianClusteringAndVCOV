# Seems to be fine. Probably need more analysis
setwd("C:/Users/Tom Kennes/Desktop/Research Master/Thesis/Core/Functions and Codes/Results Wishart Prior On Cluster")
rnormal_inverse_wishart <- function(v, W, y, m, tau){
  if(is.null(ncol(y))){
...
plot.lot <- function(y, col = c(), max_colours_value = 250, lwd = 3, main = "A Nice PlotLot"){
  m = max(y)
  n = min(y)
  if(length(col > 0)){
    colours = col
  ...
  for(i in 2:ncol(y)){
    points(y[,i], 
           col = rgb(red = colours[i,1], green = colours[i,2], blue = colours[i,3], maxColorValue = max_colours_value), type ="l",
           lwd = lwd)
  }
}
...
average.autocovariance.matrix <- function(y = ncol(y), k){
  for(i in 1:ncol(y)){
    if(i == 1){
      a = acf(y[,i], lag.max = (k - 1), plot = FALSE)$acf
    } else {
      a = cbind(a, acf(y[,i], lag.max = (k - 1), plot = FALSE)$acf)
    }
  }
  a = rowMeans(a)
...
y = as.matrix(read.csv("Kalman Filtered Series.csv"))
setwd("C:/Users/Tom Kennes/Desktop/Research Master/Thesis/Core/Functions and Codes/Results Wishart Prior On Cluster")
t = 5
covariance_mat <- average.autocovariance.matrix(y, t + 1)
plot.timewindow(y[,2:ncol(y)], pdf = "Different Time Intervals.pdf", period = t)
y = y[20:(20+t),2:ncol(y)]
plot.lot(y)
y = t(y)
text = paste("y = y[order(", paste(paste("y[,",1:ncol(y), "]", sep = ""), collapse = ", "), "),]", sep = "")
eval(parse(text = text))

s = 2
v = 75
W = covariance_mat  # set up of the wishart matrix goal. E.g. a parameter in wishart
m <- colMeans(y)
S <- W
N <- 100    # Number of Iterations in gibbs
tau = 2     # should be strictly higher than 0. Measure of confidence in the prior versus the data.
...
tmp <- rnormal_inverse_wishart(v,W, y, m,tau)
...
for(k in 2:N){
  ...
  for(i in 1:N_y){
    ...
    if(transition == max(clusters[k,])){
      ...
      tmp <- rnormal_inverse_wishart(v, W, y[i,], y[i,], tau)
      ...
      tmp <- rnormal_inverse_wishart(v, W, y[i,], colMeans(y_clust), tau)
      iterations_m[i, ,k] <- tmp$mu
      if(use_fixed_variance){
        iterations_V[, , i, k] <- diag(nrow(tmp$V))*fixed_variance
      } else {
...
pdf("Clusters over time.pdf")
for(j in 1:nrow(clusters)){
  type <- levels(as.factor(clusters[j,]))
  plot(y[clusters[j,] == type[1],1], y[clusters[j,] == type[1],2],
  ...
  for(i in 2:length(type)){
    rgb_ <- runif(n = 3, min= 0, max = 255)
    points(y[clusters[j,] == type[i],1], y[clusters[j,] == type[i],2], type = "l", col = 
             rgb(rgb_[1], rgb_[2], rgb_[3], maxColorValue = 255))
  }
}
dev.off()
...
colours <- matrix(0, nrow = length(clusters[k,]), ncol = 3)
types = as.numeric(levels(as.factor(as.character(clusters[k,]))))
for(i in 1:length(types)){
  col = runif(3, 0, 30)
  for(j in 1:length(clusters[k,])){
    if(clusters[k,j] == types[i]){
      colours[j,] = col
    }
  }
}
plot.lot(t(y), col = colours, max_colours_value = 30, main = "Clustering of Portfolios, colours are Clusters. Cluster Wishart Prior")
